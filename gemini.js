...
const genAI = new GoogleGenerativeAI(process.env.googleGemini);
...
client.on('messageCreate', async (message) => {
    try {
        ...
        // NOTE DISABLED BECAUSE WANTED TO MAKE THE RESPOND TEXT USING A SPECIFIC COMMAND
        // if (shouldRespond) {
        //     if (message.guild) {
        ...
        //     }
        // }
        if (message.content.startsWith("w!gemini")) {
            message.content = message.content.slice("w!gemini".length).trim();
            if (message.guild) {
                initializeBlacklistForGuild(message.guild.id);
                if (blacklistedUsers[message.guild.id].includes(message.author.id)) {
                    return message.reply({ content: 'You are blacklisted and cannot use this bot.' });
